...
- Currency is integer cents. Every balance change goes through `services.payment_service.adjust_balance`, which also writes a `transactions` row.
- Migrations: `alembic revision --autogenerate -m "…"` then `alembic upgrade head`. Seed idempotently via `python seed_industries.py`.
- Grant admin via `python make_admin.py <username>`.
- Env vars: `DATABASE_URL`, `JWT_SECRET`, `JWT_ALGORITHM`, `CORS_ORIGIN`.
- Python runtime lives at `~/.python312/bin/python3` (standalone install, no Xcode needed).
